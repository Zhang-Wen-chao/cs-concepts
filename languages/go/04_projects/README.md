...
3. **实现**：`playground/01_cli_service/cmd/api` + `01_cli_service/cmd/cli` + `01_cli_service/internal/bridge`，与 Stage 2/3 的组件复用。
...
- [ ] 统一的 `01_cli_service/internal/bridge` 包：抽象协议/DTO，便于 C++/Python 组件对接。
- [ ] 集成测试：`01_cli_service/cmd/api` 用 `httptest` 自定义 server，CLI 用 golden file + fake API。
...
go run 01_cli_service/cmd/api &
go run 01_cli_service/cmd/cli --query \"error\" | tee out.log
