...
- API：`01_cli_service` 中的服务部分接受批量数据，使用 `sync.RWMutex` 保护内存指标；背景 goroutine 定期 flush/持久化。
